...
if __name__ == '__main__':
    # 设置随机数种子
    random_seed = 42  # 可以使用任何整数作为种子
    random.seed(random_seed)
    # 设置训练集和测试集的比例
    train_ratio = 0.6  # 80%的图片用于训练，20%用于测试
    ...
    for center in os.listdir(data_source):
        ...
        new_folder = "%s_%s/%s"%(data_source,CT_type,name)
    ...
    workbook.save(os.path.join("%s_%s"%(data_source,CT_type),"patient_info.xls"))