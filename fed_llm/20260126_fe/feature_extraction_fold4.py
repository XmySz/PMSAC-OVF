...
class DataSet_v2(data.Dataset):
    ...
    txt 每行格式（推荐）:
        case_dir \t label
    可选扩展格式:
        case_dir \t label \t center \t leison_class

    返回与旧版保持一致:
        patient_img.copy(), np.array(size), patient_name, center_name, leisonclass, label
    """
    def __init__(self, txt_path, leison_class, center_name, duplicate_single_image=True):
        assert os.path.exists(txt_path), f"txt not found: {txt_path}"

        self.files = []  # 每一项是一个病例(case)
        self.duplicate_single_image = duplicate_single_image
        ...
        for line in lines:
            ...
            # 可选字段
            # center_name = parts[2] if len(parts) >= 3 else ""
            # leison_class = parts[3] if len(parts) >= 4 else ""
            ...
            patient_name = os.path.basename(os.path.normpath(case_dir))

            self.files.append({
                "img": case_dir,                 # 病例目录
                "label": label,                  # 直接用 txt 给的 label
                "center": center_name,
                "leison_class": leison_class,
                "patient_name": patient_name,
                "img_num": len(img_list),
                "img_list": img_list             # 预存文件名，避免getitem再扫一次目录
            })

        if len(self.files) == 0:
            raise RuntimeError(f"No valid cases found using txt: {txt_path}")
    ...
    def __getitem__(self, index):
        ...
        case_dir = datafiles["img"]
        for img in datafiles["img_list"]:
            img_path = os.path.join(case_dir, img)
            image = Image.open(img_path).convert("RGB")
            image = image.resize((224, 224))
            image = np.asarray(image, np.float32)
            ...
            # 兼容你旧代码的“单图重复一次”行为（默认关闭）
            if self.duplicate_single_image and datafiles["img_num"] == 1:
                patient_img.append(image)
                patient_label.append(label)

        patient_img = np.array(patient_img, dtype=np.float32)
        patient_label = np.array(patient_label, dtype=np.int64)

        size = patient_img.shape  # (N, 3, 224, 224)
        return patient_img.copy(), np.array(size), patient_name, center_name, leisonclass, label
...
if __name__ == '__main__':
    # RANDOM_SEED = 41
    data_total_path=r"/mnt/yxyxlab/qyj/Orthopedics/0115/perfold/fold4"
    model_total_path = \
    r"/mnt/yxyxlab/qyj/Orthopedics/llm_FL/model_save/20260120/fold4/dinov2_2026-01-22_llm_fedlwt_resnet18_Orthopedics_12/models_auc_ratio/llm_fedlwt"
    model_time = "experiment_log-2026-01-22-1053-56.pth"
    #model_time = ".pth"
    epochs = 4
    fold_id = Path(data_total_path).name
    save_path_base = r"/mnt/yxyxlab/qyj/Orthopedics/0115/fe_result/0130"
    save_path = ospj(save_path_base, fold_id)
    ...
    for epoch in range(epochs, epochs+1):
        device="cuda"
        true_epoch = epoch -1 #训练保存的模型轮次是 0 1 2 3 ... 
        wb = xlwt.Workbook(encoding='utf-8')
        # data_source=["江门医院","广东省医","湛江医院","中大五院","中山肿瘤"]
        # data_source=["CenterA(jm)", "CenterB(mm)", "CenterC(bs)", "CenterD(gz)", "CenterE(zd)"]
        data_source=["CenterE(zd)"]
        for data_name in data_source:
            train_file = data_name + "_train.txt"
            val_file = data_name + "_val.txt"
            test_file = data_name + "_test.txt"
            
            train_path = ospj(data_total_path, train_file)
            val_path = ospj(data_total_path, val_file)
            test_path = ospj(data_total_path, test_file)
            ...
            train_dataset=DataSet_v2(train_path,leison_class = "osteo",center_name = data_name)
            val_dataset = DataSet_v2(val_path, leison_class = "osteo",center_name = data_name)
            test_dataset=DataSet_v2(test_path,leison_class = "osteo",center_name = data_name)
            
            train_DL = data.DataLoader(dataset=train_dataset, batch_size=1, drop_last=False, shuffle=False,
                                       num_workers=6)
            val_DL = data.DataLoader(dataset=val_dataset, batch_size=1, drop_last=False, shuffle=False, num_workers=6)
            ...
            with torch.no_grad():
                ...
                val_epoch_Conv_feature_collector = []
                val_epoch_pred_collector = []
                val_label = []
                net.eval()
                print(epoch, data_name, "=============val_data_predicting==============")

                for batch_idx, (imgs, _, patient_name, center, leison_name, patient_label) in enumerate(val_DL):
                    sheet.write(batch_idx + plus_index, 0, patient_name[0])
                    sheet.write(batch_idx + plus_index, 1, "val_data")
                    ...
                    if imgs.numel() == 0 or imgs.ndim != 4:
                        print(f"[空图像] {patient_name[0]}，写入0特征")
                        if feature_dim is not None:
                            val_epoch_Conv_feature_collector.append(np.zeros(feature_dim, dtype=np.float32))
                        else:
                            print(f"[警告] 未知特征维度，跳过 {patient_name[0]}")
                            continue
                        val_epoch_pred_collector.append(0.0)
                        val_label.append(patient_label.data.numpy())
                        continue
                    ...
                    try:
                        ...
                        val_epoch_pred_collector.append(0.0)
                        val_label.append(patient_label.data.numpy())
                        val_epoch_Conv_feature_collector.append(np.zeros(feature_dim if feature_dim else 512, dtype=np.float32))
                        continue

                    val_epoch_pred_collector.append(mean_pred_value)
                    val_label.append(patient_label.data.numpy())
                    val_epoch_Conv_feature_collector.append(mean_feature)

                plus_index = len(train_DL)+len(val_DL)
            ...
            wb.save(os.path.join(save_path1, "name_zd.xls"))

            sio.savemat(feature_save_path, {
                "train_data": train_epoch_Conv_feature_collector,
                "train_label": train_label,
                "val_data": val_epoch_Conv_feature_collector,
                "val_label": val_label,
