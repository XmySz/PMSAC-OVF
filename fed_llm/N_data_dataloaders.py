...
def recorded_multicenters_data_API_dataloader(args):
    ...
    publish_data3=["lidc"]
    private_data=["lung_nodules","gastric","gastric_resplit1","gastric_resplit2","gastric_resplit2","Endometrial_Cancer"]
    ...
    if args.dataset in publish_data1:
        ...
        if args.dataset=='lung_nodules':
            ...
            for site in sites:
                ...
                train_dl_local = data.DataLoader(train_set, batch_size=args.batch_size, drop_last=False, shuffle=True,num_workers=8,
                                            pin_memory=True)
                test_dl_local = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False,pin_memory=True,num_workers=8)
                train_loaders.append(train_dl_local)
                test_loaders.append(test_dl_local)
                net_dataidx_map[site]=train_set
        elif 'gastric'in args.dataset:
            net_dataidx_map = {}
            ...
            for site in sites:
                ...
                test_loaders.append(test_dl_local)
                net_dataidx_map[site] = train_set
        elif args.dataset=="Endometrial_Cancer":
            net_dataidx_map = {}
            sites = ["江门", "东莞", "开平","粤北"]#, "茂名"
            for site in sites:
...
def recorded_llm_multicenters_dataloader(args):
    ...
    publish_data2 = ["cifar100", "cifar10", "tiny-imagenet"]  # lidc还没封装
    publish_data3 = ["lidc"]
    private_data2 = ["Orthopedics"]
    private_data = ["lung_nodules", "gastric", "gastric_resplit1", "gastric_resplit2", "gastric_resplit2","early_lung_nodules","Endometrial_Cancer"]
    train_loaders, test_loaders = [], []
    val_loaders = []
    trainsets, testsets = [], []
    valsets = []
    ...
    if args.dataset in private_data:
        if args.dataset == 'lung_nodules':
            ...
            for site in sites:
                # train_dl_local, test_dl_local, train_ds_local, test_ds_local=get_dataloader(args.dataset, args.datadir, args.batch_size, args.batch_size,center_name=site)
                train_set = llm_med_DataSet(root=args.datadir, center=site, data_set="train_data",patch_h=64,patch_w=64,transform=transforms.ToTensor())
                test_set = llm_med_DataSet(root=args.datadir, center=site, data_set="test_data",patch_h=64,patch_w=64,transform=transforms.ToTensor())
                train_dl_local = data.DataLoader(train_set, batch_size=args.batch_size, drop_last=False, shuffle=True,num_workers=8,
                                                pin_memory=True)
                test_dl_local = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False,num_workers=8,
                                                pin_memory=True)
                ...
                test_loaders.append(test_dl_local)
                net_dataidx_map[site] = train_set
        elif args.dataset == 'early_lung_nodules':
            net_dataidx_map = {}
            ...
            for site in sites:
                ...
                test_dl_local = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=16,
                                                pin_memory=True)
                train_loaders.append(train_dl_local)
                test_loaders.append(test_dl_local)
                net_dataidx_map[site] = train_set
            ...
            for site in sites:
                ...
                test_loaders.append(test_dl_local)
                net_dataidx_map[site] = train_set
    if args.dataset in private_data2:
        if args.dataset == "Orthopedics":
            net_dataidx_map = {}
            sites = ["CenterA(jm)", "CenterB(mm)", "CenterC(bs)", "CenterD(gz)", "CenterE(zd)"]
            for site in sites:
                train_set = llm_med_DataSet(root=args.datadir, center=site, data_set="train",patch_h=64,patch_w=64,transform=transforms.ToTensor())
                test_set = llm_med_DataSet(root=args.datadir, center=site, data_set="test",patch_h=64,patch_w=64,transform=transforms.ToTensor())
                train_dl_local = data.DataLoader(train_set, batch_size=args.batch_size, drop_last=False, shuffle=True,num_workers=4,pin_memory=True)
                test_dl_local = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False,num_workers=4,pin_memory=True)
                train_loaders.append(train_dl_local)
                test_loaders.append(test_dl_local)
